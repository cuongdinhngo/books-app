  ...
  if (quantity.value > 0 && oldQuantity.value !== quantity.value) {
    ...
    if (quantity.value < oldQuantity.value && quantity.value >= fixedCounts) {
      ...
      if (pendingCounts.value >= removeCounts) {
        await getBookItems({ columns:'id', bookId: bookId.value, status: BOOK_STATUS.PENDING })
        ...
        await deleteBookItems({ bookId: bookId.value, status: BOOK_STATUS.PENDING})
          .then(async({ error }) => {
            if (error) throw error;
            await getBookItems({ columns:'id', bookId: bookId.value, status: BOOK_STATUS.OPENING })
              .order('id', {ascending:true})
              .limit(openingRemove)
    ...
    await getBookItems({columns: 'id,status', bookId: bookId.value})
      .then(({ data, error }) => {
        if (error) throw error;
